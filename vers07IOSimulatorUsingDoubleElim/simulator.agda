...
mutual
   myProgram : ∀ {i} → (c0 : Choice) 
               → Process ∞  (ChoiceSet c0)  
               → IOConsole i Unit  
   ...
   myProgram0 : ∀ {i} → (c0 : Choice) 
               → Process ∞  (ChoiceSet c0) → Bool → Bool  
               → IOConsole i Unit  
...
-- choiceIsEmpty
   ...
   myProgram1 : ∀ {i} → (c0 : Choice) → Process ∞ (ChoiceSet c0) 
                      → IOConsole i Unit 
   forceIO (myProgram1 c0 P) = do' getLine λ s → myProgram2a c0 P s (s ==strb "quit")


   myProgram2a : ∀ {i} → (c0 : Choice) 
                 → (P : Process ∞ (ChoiceSet c0)) → String → Bool  
                 → IOConsole i Unit  
   ...
   forceIO (myProgram2 c0 P nothing) = do' (putStrLn "please enter a choice amongst") λ _ →  
                                       do (putStrLn (showLabels₁ P)) λ _ → 
                                       myProgram1 c0 P
   myProgram2 c0 P (just c1) =  myProgram c0 (forcep (processToExternalSubprocess0 P c1) ∞ )

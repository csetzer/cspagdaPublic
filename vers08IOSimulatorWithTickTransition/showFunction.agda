...
choiceFunctionToString : {A : Set} → (f : A → String) → (c : Choice) → (g : ChoiceSet c → A) → String
choiceFunctionToString {A} f c g = unlinesWithChosenString " " (mapL  (λ x →  "(λ " ++s choice2String c x ++s " → " ++s f (g x) ++s ")")  (choice2Enumeration0 c))

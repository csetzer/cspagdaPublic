...
choice2String (c₀ +'' c₁) (inl a) = "inl(" ++s (choice2String c₀ a) ++s ")"
choice2String (c₀ +'' c₁) (inr a) = "inr(" ++s (choice2String c₁ a) ++s ")"
...
boolToMaybeTrue : (b : Bool) → Maybe (True b)
boolToMaybeTrue false = nothing
boolToMaybeTrue true = just triv

set2MaybeSubset0  : (A : Set) → (f : A → Bool) → (a : A) → Maybe (True (f a))  → Maybe (subset A f)
...
choice2Enumeration0 (c₀ +'' c₁) = mapL (λ a → inl a) (choice2Enumeration0 c₀) ++ 
                                  mapL (λ a → inr a) (choice2Enumeration0 c₁) 

...
--- this is currently only a partial version of vers1UsingCodata
...
{- ideal notation for first part

   coalg Process5 (i : Size) (A : Set) : Set where
       force : {j : Size< i} → Process5' j A
-}
...
record sizedStream (i : Size) : Set where
  coinductive
  field
    head' : ℕ 
    tail' : {j : Size< i} → sizedStream j
...
inc : ℕ → Stream
head (inc n) = n
tail (inc n) = inc (suc n)


mutual
   record coList : Set where
     coinductive
     field 
       unfold : coListShape
   ...
   data coListShape : Set where
     nil : coListShape
     cons : ℕ → coList → coListShape

open coList


l : coList
unfold l = cons zero l
...
mutual
   record sizedCoList {i : Size} : Set where
     coinductive
     field 
       unfold' : {j : Size< i} → sizedCoListShape {j}

   data sizedCoListShape : {i : Size} → Set where
     nil : {i : Size} → sizedCoListShape {i}
     cons : {i : Size} → ℕ → sizedCoList {i}  → sizedCoListShape {i}

open sizedCoList

append : {i : Size} → sizedCoList {i} → sizedCoList {i} → sizedCoList {i}
unfold' (append l l') with unfold' l
... | nil = unfold' l'
...
a1 : sizedCoList
unfold' a1 = cons zero a0

a2 : sizedCoList
unfold' a2 = cons (suc zero) a1

a : (i : Size) → sizedCoList {i}
unfold' (a i) {j} = cons zero (append {j} a2 (a j))


mutual
   record Process5 (i : Size) (A : Set) : Set where
     coinductive
     constructor delay
     field
       force : {j : Size< i} → Process5shape j A

   
   data Process5shape (i : Size) ( A : Set ) : Set where
       node : (E   : ExtChoice) 
           → (Lab : ExtChoice2Set E → ExtLabel)
           → (PE  : ExtChoice2Set E → Process5 i A) 
           → (I   : IntChoice)
           → (PI  : IntChoice2Set I → Process5 i A)
           → Process5shape i A
       terminate : A → Process5shape i A

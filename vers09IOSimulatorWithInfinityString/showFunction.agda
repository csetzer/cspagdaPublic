...
showLabels₁ (node P) = printOptionAux {E P} {I P} (choice2EnumWithStr (E P)) (Lab P) (choice2EnumWithStr (I P))
...
funcChoice2ProcessToProcess∞ : ∀ {i} → (c d : Choice) → (ChoiceSet c  → Process∞ (↑ i)  d) → String
funcChoice2ProcessToProcess∞ {i} c d f = funcChoice2ProcessToProcess c d (λ x → forcep (f x ) )
...
funcChoice2ProcessToProcess₁∞ : ∀ {i}  → {c₀ : Choice} → (c : Choice) 
                                → (ChoiceSet c  → Process∞ (↑ i)  c₀) 
                                → String
funcChoice2ProcessToProcess₁∞ {i} c f = funcChoice2ProcessToProcess₁ c (λ x → forcep (f x )  )
...
showProcess∞ : {i : Size} →  {c : Choice} → Process∞  (↑ i) (ChoiceSet c)   → String
showProcess∞ {i} {c} P = showProcess {i} c (forcep P )
-}

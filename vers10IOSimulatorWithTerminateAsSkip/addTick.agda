...
add✓Str : {c : Choice} → (a : ChoiceSet c)  → String  → String
add✓Str a str = "(add✓ " ++s choice2Str a ++s " " ++s str ++s ")"

mutual
  add✓∞ : ∀ {i} → {c : Choice} → (a : ChoiceSet c) → Process∞ i c → Process∞  i c
  ...
  add✓+ : ∀ {i} → {c : Choice} → (a : ChoiceSet c) → Process+ i c 
              → Process+ i c 
  E    (add✓+ a P)    = E P
  Lab  (add✓+ a P)    = Lab P 
...
mutual
  ...
  addTimed✓+ : ∀ {i} → {c : Choice} → (a : ChoiceSet c) → Process+ i c 
              → Process+ i c 
  E    (addTimed✓+ a P)    = E P
